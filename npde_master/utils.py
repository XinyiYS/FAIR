...
def gen_data(model='vdp',Ny=[30],tbegin=0,tend=8,x0=np.asarray([2.0,-3.0]),nstd=0.1, plot=False):
    x0in = x0
    Nt = len(Ny)
    x0 = np.zeros((Nt,2))
    t = np.zeros((Nt,), dtype=np.object)
    ...
    diff = lambda x,t: ss.norm.rvs(size=[2,1]) * gtrue(x,t)
    for i in range(Nt):
        tspan = np.linspace(tbegin,tend,Ny[i])
        t[i] = tspan
        X = em_int(vdp, diff, x0in, tspan)
        Y[i] = X + ss.norm.rvs(size=X.shape) * nstd
        x0[i,:] = Y[i][0,:]
...
def eval_model(npde,t,Y,Nw=1):
    ...
    if npde.name is 'npode':
        ...
        rmse_overD = 0
        for d in range(D):
            rmse = np.mean([(pred - true)**2 for (pred, true) in zip(X[:,d], true_path[:,d])] )
            rmse_overD += rmse
        rmse_overD /= D
        ...
        for d in range(D):
            ...
            rmse = []
            for i in range(X[0].shape[0]):
                rmse.append(np.mean( [(pred - true)**2 for (pred, true) in zip(X[0][i,:, d], true_vdp[:, d])]) )
            rmse_overD += np.mean(rmse)

        rmse_overD /= D

    print("RMSE over the dimensions is:", rmse_overD)
    return rmse_overD
