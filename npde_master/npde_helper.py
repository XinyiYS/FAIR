...
from npde_master.npde import NPODE, NPSDE, BrownianMotion
from npde_master.kernels import OperatorKernel
from npde_master.utils import plot_model
...
def fit_model(sess,model,t,Y,Nw=10,num_iter=500,print_every=10,eta=5e-3,dec_step=20,dec_rate=0.99,plot_=True,verbose=False):
    ...
    if model.name is 'npode':
        with tf.name_scope("cost"):
            ...
            for i in range(len(X)):
                mvn = tfp.distributions.MultivariateNormalFullCovariance(loc=X[i],covariance_matrix=tf.linalg.tensor_diag(model.sn))
        ...
        with tf.name_scope("cost"):
            ...
            ll = 0
            for i in range(len(Y)):
                mvn = tfp.distributions.MultivariateNormalFullCovariance(loc=Y[i],covariance_matrix=tf.linalg.tensor_diag(model.sn))
    ...
    for i in range(1,num_iter+1):
        ...
        if verbose and (i==1 or i%print_every==0 or i==num_iter):
