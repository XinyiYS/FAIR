...
from tfp import distributions as tfd
# import tensorflow.contrib.distributions as tfd
from npde_master.integrators import ODERK4, SDEEM
from npde_master.kernels import OperatorKernel

from gpflow import transforms
from npde_master.param import Param
...
class NPODE:
    ...
    def U(self):
        U = self.U
        if self.whiten:
            Lz = tf.linalg.cholesky(self.Kzz())
            U = tf.matmul(Lz,U)
        return U

    def __str__(self):
        rep = 'name:                  ' + str(self.name) + \
            '\nnoise variance:        ' + str(self.sn.eval()) + \
