...
def run_single_experiment(results_dir, index):
    ...
    de_model, t, Y, prior_sn, posterior_sn = generate_vdp_data_and_fit_model(index)
    ...
    with cwd(results_dir):
      np.savez(f'trial-{index}-setting-VDP-time-{time_str}', t=t, Y=Y, Ts=Ts, Ss=Ss, times=times, 
...
if __name__ == '__main__':
    ...
    n_trials = 5
    print(f'Start running {n_trials} trials for DE-VDP experiments.')
